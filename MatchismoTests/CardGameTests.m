...
- (void)testSimple
{
    ...
    PokerMatchCardGame *game = [[PokerMatchCardGame alloc]initWithCardCount:4 fromDeck:deck shouldShuffle:NO matching:2];
    
    [game flipCardAtIndex:0];
    STAssertEquals(game.score, -1, @"Flip one card, -1 as flip cost");
    
    STAssertEquals([[game cardAtIndex:0] isPlayable], YES, @"Card expect to be playable.");
    STAssertEquals([[game cardAtIndex:0] isFaceUp], YES, @"Card still face up");
    
    [game flipCardAtIndex:1];
    STAssertEquals([[game cardAtIndex:0] isPlayable], NO, @"Card expect to be playable.");
    STAssertEquals([[game cardAtIndex:0] isFaceUp], YES, @"Card still face up");
    STAssertEquals([[game cardAtIndex:1] isPlayable], NO, @"Card expect to be playable.");
...
- (void)testMatch3Cards
{
    ...
    PokerMatchCardGame *game = [[PokerMatchCardGame alloc]initWithCardCount:4 fromDeck:deck shouldShuffle:NO matching:3];
    
    [game flipCardAtIndex:3];
    STAssertEquals(game.score, -1, @"Flip one card, another -1 as flip cost");
    STAssertEquals([[game cardAtIndex:3] isPlayable], YES, @"Card expect to be playable.");
    ...
    STAssertEquals([[game cardAtIndex:0] isPlayable], YES, @"Card expect to be playable.");
    STAssertEquals([[game cardAtIndex:0] isFaceUp], YES, @"Card still face up");
    STAssertEquals([[game cardAtIndex:1] isPlayable], YES, @"Card expect to be playable.");
    STAssertEquals([[game cardAtIndex:1] isFaceUp], YES, @"Card still face up");
    ...
    STAssertEquals([[game cardAtIndex:1] isPlayable], NO, @"Card expect to be playable.");
    STAssertEquals([[game cardAtIndex:1] isFaceUp], YES, @"Card still face up");
    STAssertEquals([[game cardAtIndex:2] isPlayable], NO, @"Card expect to be playable.");
